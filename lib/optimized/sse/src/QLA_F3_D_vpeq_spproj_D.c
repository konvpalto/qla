...
void
QLA_F3_D_vpeq_spproj_D( QLA_F3_DiracFermion *restrict r,
...
#pragma omp parallel for
	for(int i=0; i<n; i++) {
	  ...
	  for(i_c=0; i_c<3; i_c++) {
	    ...
	    storeaps(foff(ri,8*i_c), addps(loadaps(foff(ri,8*i_c)), t1));
	    sprecon0p(t2, t1);
	    storeaps(foff(ri,8*i_c+4), addps(loadaps(foff(ri,8*i_c+4)), t2));
	  }
	}
      } break;
      case 1: {
...
#pragma omp parallel for
	for(int i=0; i<n; i++) {
	  ...
	  for(i_c=0; i_c<3; i_c++) {
	    ...
	    storeaps(foff(ri,8*i_c+4), addps(loadaps(foff(ri,8*i_c+4)), t1));
	  }
	}
      } break;
      }
